...
# Run CLI commands
pnpm start init      # Initialize submodules
pnpm start sync      # Sync all skills from submodules
pnpm start check     # Check for updates
pnpm start cleanup   # Remove unused submodules/skills

# With skip prompts flag
pnpm start sync -y
```

**Note:** This project has no test suite. The root only contains CLI scripts and skill generation logic.

## Code Style

This project uses `@antfu/eslint-config` with strict TypeScript settings.
...
- Enable `strictNullChecks` - always handle null/undefined cases

### Imports & Formatting

- Use ESM imports (`import { x } from 'x'`)
- Use `@/` path aliases when configured (for project internal imports)
- Run `pnpm lint` before committing - it auto-fixes most issues
- Follow the formatter output (single quotes, trailing commas, etc.)

### Naming Conventions

- **Files**: `kebab-case.ts` for scripts, `PascalCase.ts` for components/classes
- **Functions**: `camelCase` - use verb prefixes (`get`, `set`, `is`, `has`, `create`)
...
- Use custom error types when appropriate for better error categorization
...
PLEASE STRICTLY FOLLOW THE BEST PRACTICES FOR SKILL: https://platform.claude.com/docs/en/agents-and-tools/agent-skills/best_practices
...
- **Projects:** Vue, Nuxt, Vite, UnoCSS, Pinia, Vitest, VitePress, Nitro, pnpm
...
- **Projects:** Slidev, VueUse, Vercel Labs, Anthropics, Turborepo, etc.
...
> **Note:** The `skills/` directory is NOT tracked in git. It is generated by running `pnpm start sync`. This keeps the repository size small and allows automatic updates from upstream vendors.
